...
public class CropFragment extends Fragment {
	...
	static {
		// init data
		dataList.add(new RatioItem("Ratio", -1f));
	...
	private final class CropImageTask extends AsyncTask<Bitmap, Void, Bitmap> {
		...
		protected Bitmap doInBackground(Bitmap... params) {
			RectF cropRect = mCropPanel.getCropRect();// Cut area rectangle
			Matrix touchMatrix = activity.mainImage.getImageViewMatrix();
			// Canvas canvas = new Canvas(resultBit);
			float[] data = new float[9];
			touchMatrix.getValues(data);// The bottom picture changes the record matrix raw data
			Matrix3 cal = new Matrix3(data);// Auxiliary matrix calculation class
			Matrix3 inverseMatrix = cal.inverseMatrix();// Calculate the inverse matrix
			Matrix m = new Matrix();
			m.setValues(inverseMatrix.getValues());
			m.mapRect(cropRect);// Change the cut rectangle
