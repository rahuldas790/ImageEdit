...
public class FliterListFragment extends Fragment {
    ...
    private void setUpFliters() {
        ...
        Bitmap bmp = Bitmap.createBitmap(activity.mainBitmap.copy(
                Bitmap.Config.RGB_565, false));
        Bitmap thumb = null;
        for (int i = 0, len = fliters.length; i < len; i++) {

            View view = LayoutInflater.from(activity).inflate(R.layout.custom_filter_layout, null);
            TextView text = (TextView) view.findViewById(R.id.text);
            text.setText(fliters[i]);
            ImageView imageView = (ImageView) view.findViewById(R.id.image);

            thumb = ThumbnailUtils.extractThumbnail(bmp, THUMBSIZE, THUMBSIZE);

            Bitmap bit = PhotoProcessing.filterPhoto(thumb, i);
            imageView.setImageBitmap(bit);
            imageView.setTag(i);
            imageView.setOnClickListener(new FliterClick());
            mFilterGroup.addView(view, params);
